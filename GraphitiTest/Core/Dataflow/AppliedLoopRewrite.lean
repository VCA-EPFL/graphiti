...
noncomputable def LoopRewrite_Environment
    ...
    {st _st' : RewriteState String (String × Nat)}
    {elems : List String}
    {types : Vector (String × Nat) LoopRewrite.rewrite.params}
    (h_wf : ε_global.toEnv.well_formed)
    (h1 : ε_global.max_typeD <= st.fresh_type.2)
...
/- /--
 ...
 -     [Ge| g', (ε_global ++ (VerifiedRewrite.ε_ext (@LoopRewrite.verified_rewrite (LoopRewrite_Environment h_wf h1 h2 h3 h4 h5 h6)))).toEnv ] ⊑ [Ge| g, ε_global.toEnv ] :=
 -   by apply run'_refines Env.well_formed h2 h3 h4 h5 h6
 -
