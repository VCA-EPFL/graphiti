...
def identMatcher (join2 : String) : Pattern String (String × Nat) 2 := fun g => do
  ...
  unless "join" == join2_typ.2.1 do throw (.error s!"type of '{join2}' is '{join2_typ.2}' instead of 'join'")
  let join1 ← ofOption' (.error s!"{decl_name%}: could not find next node") <| followInput g join2 "in1"
  unless "join" == join1.typ.1 do throw (.error s!"type of '{join1.inst}' is '{join1.typ}' instead of 'join'")
