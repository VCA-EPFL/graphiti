...
def identMatcher (s : String) (g : ExprHigh String String) : RewriteResult (List String × List String) := do
  ...
  let (.some t1) := n.2.splitOn[1]? | throw (.error s!"{decl_name%}: type incorrect3")
  let (.some t2) := next.typ.splitOn[1]? | throw (.error s!"{decl_name%}: type incorrect1")
  let (.some t3) := next.typ.splitOn[2]? | throw (.error s!"{decl_name%}: type incorrect2")
