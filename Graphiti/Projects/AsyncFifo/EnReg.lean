...
is written down. -/
def drv (clk : List Bool) (en : List Bool) (data : List Bool) (crn : List Bool) : Drv W
...
theorem drv_mono {clk en data crn} :
    Mono (drv clk en data crn) := by
  intro a b h k
...
theorem drv_env {clk clk' : List Bool} {en en' : List Bool} {data data' : List Bool} {crn crn' : List Bool}
    (hclk : clk <+: clk') (hen : en <+: en') (hdata : data <+: data') (hcrn : crn <+: crn') (w : Wires W) (k : W) :
    drv clk en data crn w k <+:
      drv clk' en' data' crn' w k := by
  cases k <;> simp only [drv]
...
section Clauses
variable {clk en data crn : List Bool} {w : Wires W} (hw : Wf (drv clk en data crn) w)
include hw
...
theorem wf_t1_b : w .t1_b <+: data := hw .t1_b
...
theorem wf_cut_r3 : w .cut_r3 <+: data := hw .cut_r3
...
theorem wf_sim {clk en data crn : List Bool} {w : Wires W} (hw : Wf (drv clk en data crn) w) : ∀ t,
    (t < (w .n2_a).length → (w .n2_a).getD t false = (enRun clk en data crn t).n1) ∧
    (t < (w .n2f_in).length → (w .n2f_in).getD t false = (enRun clk en data crn t).n2) ∧
    (t < (w .n3f_in).length → (w .n3f_in).getD t false = (enRun clk en data crn t).n3) ∧
    (t < (w .n4f_in).length → (w .n4f_in).getD t false = (enRun clk en data crn t).n4) ∧
    (t < (w .n5f_in).length → (w .n5f_in).getD t false = (enRun clk en data crn t).n5) ∧
    (t < (w .n5_b).length → (w .n5_b).getD t false = (enRun clk en data crn t).n6) ∧
    (t < (w .qf_in).length → (w .qf_in).getD t false = (enRun clk en data crn t).q) ∧
    (t < (w .t2_a).length → (w .t2_a).getD t false = (enRun clk en data crn t).nen) ∧
    (t < (w .mx_a).length → (w .mx_a).getD t false = (enRun clk en data crn t).t1) ∧
    (t < (w .mx_b).length → (w .mx_b).getD t false = (enRun clk en data crn t).t2) ∧
    (t < (w .n4_b).length → (w .n4_b).getD t false = (enRun clk en data crn t).m) := by
  intro t
  induction t with
  | zero =>
...
theorem out_q {clk en data crn : List Bool} {w : Wires W} (hw : Wf (drv clk en data crn) w) :
    (w .cut_in).take (min (min (w .cut_r1).length (w .cut_r2).length) (min (w .cut_r3).length (w .cut_r4).length) + 1)
      <+: enOut clk en data crn := by
...
syntax "en_case " term : tactic
set_option hygiene false in
macro_rules
  | `(tactic| en_case $w:term) => `(tactic| (
      ...
      refine ⟨s, existSR_reflexive, ?_, e0, e1, e2, e3⟩
      · have key := Wf_set drv_mono hw $w _ (‹_ ⊏ _›).isPrefix (by
          ...
          first
            ...
            | exact (‹_ ⊏ _›).isPrefix
            | assumption)
        -- `drv` deliberately out of the simp set: unfolding it once per wire per
        -- connection is quadratic, and none of it is needed here.
...
-- 35 wires and 35 connections: re-establishing the invariant is quadratic in the
-- wire count, so this block needs a larger budget than the registers did.
  ...
  · intro ident mid_i v Hrule
    ...
    all_goals first
      | exact ⟨_, _, spec_in_clk s _ (by rw [← e0]; exact hpre), existSR_reflexive,
          Wf_drv hw (drv_env (e0 ▸ hpre.isPrefix) List.prefix_rfl List.prefix_rfl List.prefix_rfl _), rfl, e1, e2, e3⟩
      | exact ⟨_, _, spec_in_en s _ (by rw [← e1]; exact hpre), existSR_reflexive,
          Wf_drv hw (drv_env List.prefix_rfl (e1 ▸ hpre.isPrefix) List.prefix_rfl List.prefix_rfl _), e0, rfl, e2, e3⟩
      | exact ⟨_, _, spec_in_data s _ (by rw [← e2]; exact hpre), existSR_reflexive,
          Wf_drv hw (drv_env List.prefix_rfl List.prefix_rfl (e2 ▸ hpre.isPrefix) List.prefix_rfl _), e0, e1, rfl, e3⟩
      | exact ⟨_, _, spec_in_clrn s _ (by rw [← e3]; exact hpre), existSR_reflexive,
          Wf_drv hw (drv_env List.prefix_rfl List.prefix_rfl List.prefix_rfl (e3 ▸ hpre.isPrefix) _), e0, e1, e2, rfl⟩
  · intro ident mid_i v Hrule
    obtain ⟨⟨n2_a, n2_b, n2_c⟩, enf_in, n5f_in, ⟨qg_a, qg_b⟩, qf_in, ⟨t1_a, t1_b⟩, crf_in, ⟨n5_a, n5_b⟩, ⟨n6_a, n6_b, n6_c⟩, ⟨n4_a, n4_b, n4_c⟩, n4f_in, ⟨n3_a, n3_b, n3_c⟩, ⟨n1_a, n1_b⟩, n3f_in, dataf_in, ⟨t2_a, t2_b⟩, clkf_in, n2f_in, ⟨cut_in, cut_r1, cut_r2, cut_r3, cut_r4⟩, ⟨mx_a, mx_b⟩, nen_a⟩ := i
    obtain ⟨⟨_, _, _⟩, _, _, ⟨_, _⟩, _, ⟨_, _⟩, _, ⟨_, _⟩, ⟨_, _, _⟩, ⟨_, _, _⟩, _, ⟨_, _, _⟩, ⟨_, _⟩, _, _, ⟨_, _⟩, _, _, ⟨_, _, _, _, _⟩, ⟨_, _⟩, _⟩ := mid_i
    obtain ⟨hw, e0, e1, e2, e3⟩ := H
    ...
    exact ⟨s, _, existSR_reflexive, spec_out_q s _ (out_q hw), hw, e0, e1, e2, e3⟩
