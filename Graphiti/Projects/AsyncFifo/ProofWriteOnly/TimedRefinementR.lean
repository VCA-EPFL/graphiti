...
instance : MatchInterface (rdomTimed α n lat kq su stl dmin dmax ddmin ddmax Pg pwg Rg Rc)
    (rdomSpec α n lat stl su kq rdly P S R pw) := by
  dsimp [rdomTimed, rdomSpec]
  solve_match_interface

/-! ### Monotonicity in the bank's inputs -/

theorem rnextLen_mono {s s' : ReadNext.RNextSt n} (h1 : s.st <+: s'.st) (h2 : s.inc <+: s'.inc)
    (h3 : s.q1 <+: s'.q1) : ReadNext.rnextLen s ≤ ReadNext.rnextLen s' := by
  have := h1.length_le; have := h2.length_le; have := h3.length_le
  unfold ReadNext.rnextLen; omega

theorem rnextDep_congr {s s' : ReadNext.RNextSt n} (h1 : s.st <+: s'.st) (h2 : s.inc <+: s'.inc)
    (h3 : s.q1 <+: s'.q1) {u : Nat} (hu : u < ReadNext.rnextLen s) : ReadNext.rnextDep s u = ReadNext.rnextDep s' u := by
  unfold ReadNext.rnextLen at hu
  simp only [Nat.lt_min] at hu
  unfold ReadNext.rnextDep
  rw [h1.getD_eq_left (by omega), h2.getD_eq_left (by omega), h3.getD_eq_left (by omega)]

theorem _root_.Graphiti.AsyncFifo.Contracts.CombOut.rmono_next {s s' : ReadNext.RNextSt n} (h1 : s.st <+: s'.st) (h2 : s.inc <+: s'.inc)
    (h3 : s.q1 <+: s'.q1) {v : List (RNext n)}
    (h : CombOut (ReadNext.rnextDep s) ReadNext.rnextFun (ReadNext.rnextLen s) dmin dmax v) :
    CombOut (ReadNext.rnextDep s') ReadNext.rnextFun (ReadNext.rnextLen s') dmin dmax v :=
  h.mono (rnextLen_mono h1 h2 h3) (fun u hu => rnextDep_congr h1 h2 h3 hu)

theorem _root_.Graphiti.AsyncFifo.Contracts.RegOutG.rmono_bus {G : Nat → Prop} {rclk rclk' : List Bool} {rd rd' : List (RNext n)}
...
theorem _root_.Graphiti.AsyncFifo.Contracts.BusRegOutG.rmono_bus {G : Nat → Prop} {rclk rclk' : List Bool} {rd rd' : List (RNext n)}
...
theorem _root_.Graphiti.AsyncFifo.Contracts.RegOutG.rmono_guard {G G' : Nat → Prop} {rclk : List Bool} {rd : List (RNext n)}
...
theorem _root_.Graphiti.AsyncFifo.Contracts.BusRegOutG.rmono_guard {G G' : Nat → Prop} {rclk : List Bool} {rd : List (RNext n)}
...
structure RPsi (lat kq su stl dmin dmax ddmin ddmax rdly P S R pw Pg pwg Rg Rc : Nat) (crn : List Bool)
  ...
  sclk_F : sclk <+: F
  rclk_F : rclk <+: F
  q1_sync : nq1 <+: SyncStage.syncOut lat su stl sclk sd sorc
  stf_q : stf <+: stq
  st_stf : nst <+: stf
  mst_stf : mst <+: stf
  d_link : rd <+: nd
  ...
  d_ok : CombOut (ReadNext.rnextDep ⟨nst, ninc, nq1, nd⟩) ReadNext.rnextFun (ReadNext.rnextLen ⟨nst, ninc, nq1, nd⟩) dmin dmax nd
...
theorem rint_next_regs (v : List (RNext n)) (h1 : nd <+: v)
    (h2 : CombOut (ReadNext.rnextDep ⟨nst, ninc, nq1, nd⟩) ReadNext.rnextFun (ReadNext.rnextLen ⟨nst, ninc, nq1, nd⟩) dmin dmax v)
    (h3 : rd ⊏ v) :
...
theorem rspec_in_clk (v : List Bool) (h : sp.clk ⊏ v) :
    ((rdomSpec α n lat stl su kq rdly P S R pw).inputs.getIO ↑"clk").2 sp v { sp with clk := v } := by
  rw [PortMap.rw_rule_execution (by dsimp [reducePortMapgetIO])]; exact ⟨h, rfl⟩
theorem rspec_in_inc (v : List Bool) (h : sp.inc ⊏ v) :
    ((rdomSpec α n lat stl su kq rdly P S R pw).inputs.getIO ↑"inc").2 sp v { sp with inc := v } := by
  rw [PortMap.rw_rule_execution (by dsimp [reducePortMapgetIO])]; exact ⟨h, rfl⟩
theorem rspec_in_wgray (v : List (BitVec (n+1))) (h : sp.wgray ⊏ v) :
    ((rdomSpec α n lat stl su kq rdly P S R pw).inputs.getIO ↑"wgray").2 sp v { sp with wgray := v } := by
  rw [PortMap.rw_rule_execution (by dsimp [reducePortMapgetIO])]; exact ⟨h, rfl⟩
theorem rspec_in_orc (v : List (Orc n)) (h : sp.orc ⊏ v) :
    ((rdomSpec α n lat stl su kq rdly P S R pw).inputs.getIO ↑"orc").2 sp v { sp with orc := v } := by
  rw [PortMap.rw_rule_execution (by dsimp [reducePortMapgetIO])]; exact ⟨h, rfl⟩
theorem rspec_in_mem (v : List (BitVec n → α)) (h : sp.mem ⊏ v) :
    ((rdomSpec α n lat stl su kq rdly P S R pw).inputs.getIO ↑"mem").2 sp v { sp with mem := v } := by
  rw [PortMap.rw_rule_execution (by dsimp [reducePortMapgetIO])]; exact ⟨h, rfl⟩

theorem rspec_out_gray (v : List (BitVec (n+1))) (h1 : sp.gray_q <+: v)
    (h2 : RGrayF lat stl su kq P S R pw sp.clk sp.inc sp.wgray sp.orc v) :
    ((rdomSpec α n lat stl su kq rdly P S R pw).outputs.getIO ↑"gray").2 sp v { sp with gray_q := v } := by
...
theorem rspec_out_empty (v : List Bool) (h1 : sp.empty_q <+: v)
    ...
    ((rdomSpec α n lat stl su kq rdly P S R pw).outputs.getIO ↑"empty").2 sp v { sp with empty_q := v } := by
  rw [PortMap.rw_rule_execution (by dsimp [reducePortMapgetIO])]; exact ⟨h1, h2, rfl⟩
theorem rspec_out_rdata (v : List α) (h1 : sp.rdata_q <+: v)
    ...
    ((rdomSpec α n lat stl su kq rdly P S R pw).outputs.getIO ↑"rdata").2 sp v { sp with rdata_q := v } := by
  rw [PortMap.rw_rule_execution (by dsimp [reducePortMapgetIO])]; exact ⟨h1, h2, rfl⟩
...
theorem rrefines_psi (hP1 : kq + su + dmax + 2 ≤ P) (hP2 : stl + su + dmax + 2 ≤ P)
    ...
    rdomTimed α n lat kq su stl dmin dmax ddmin ddmax Pg pwg Rg Rc ⊑_{rpsi lat kq su stl dmin dmax ddmin ddmax rdly P S R pw Pg pwg Rg Rc} rdomSpec α n lat stl su kq rdly P S R pw := by
  intro i s Hψ
  ...
  · intro ident mid_i v Hrule
    ...
    all_goals simp only [Prod.mk.injEq, ReadNext.RNextSt.mk.injEq, ReadPort.RDataSt.mk.injEq, ReadBank.RRegSt.mk.injEq,
      and_assoc] at Hrule
...
theorem rrefines_initial :
    Module.refines_initial (rdomTimed α n lat kq su stl dmin dmax ddmin ddmax Pg pwg Rg Rc) (rdomSpec α n lat stl su kq rdly P S R pw)
      (rpsi lat kq su stl dmin dmax ddmin ddmax rdly P S R pw Pg pwg Rg Rc) := by
  intro i hi
  obtain ⟨csrc, stf, ⟨sclk, sd, sorc, sq⟩, F, ⟨nst, ninc, nq1, nd⟩, ⟨mst, rmem, rq⟩, ⟨rclk, rd, crn, stq, emq, gq⟩⟩ := i
  simp only [rdomTimed, Prod.mk.injEq, ReadNext.RNextSt.mk.injEq, ReadPort.RDataSt.mk.injEq, ReadBank.RRegSt.mk.injEq,
    and_assoc] at hi
...
theorem rdomTimed_refines (hP1 : kq + su + dmax + 2 ≤ P) (hP2 : stl + su + dmax + 2 ≤ P)
    ...
    (hrr : ddmin ≤ ddmax) (hrw : ddmax ≤ rdly)
    (hPg : Pg ≤ P) (hpwg : pwg ≤ pw) (hRg : Rg ≤ R) :
    rdomTimed α n lat kq su stl dmin dmax ddmin ddmax Pg pwg Rg Rc ⊑ rdomSpec α n lat stl su kq rdly P S R pw :=
