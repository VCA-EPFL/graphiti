...
`wdomSpec` with clock-period filter `P` and input-setup filter `S`, whenever
...
instance : MatchInterface (wdomTimed α n lat kq su stl dmin dmax Pg pwg Rg Rc) (wdomSpec α n lat stl su kq P S R pw) := by
  dsimp [wdomTimed, wdomSpec]
  solve_match_interface

/-! ### Monotonicity of the next-state contract in the block's inputs -/

theorem nextLen_mono {s s' : WriteNext.NextSt α n} (h1 : s.st <+: s'.st) (h2 : s.inc <+: s'.inc) (h3 : s.data <+: s'.data)
    (h4 : s.q1 <+: s'.q1) : WriteNext.nextLen α s ≤ WriteNext.nextLen α s' := by
  have := h1.length_le; have := h2.length_le; have := h3.length_le; have := h4.length_le
  unfold WriteNext.nextLen; omega

theorem nextDep_congr {s s' : WriteNext.NextSt α n} (h1 : s.st <+: s'.st) (h2 : s.inc <+: s'.inc) (h3 : s.data <+: s'.data)
    (h4 : s.q1 <+: s'.q1) {u : Nat} (hu : u < WriteNext.nextLen α s) : WriteNext.nextDep α s u = WriteNext.nextDep α s' u := by
  unfold WriteNext.nextLen at hu
  unfold WriteNext.nextDep
  rw [h1.getD_eq_left (by omega), h2.getD_eq_left (by omega), h3.getD_eq_left (by omega), h4.getD_eq_left (by omega)]

theorem _root_.Graphiti.AsyncFifo.Contracts.CombOut.mono_next {s s' : WriteNext.NextSt α n} (h1 : s.st <+: s'.st) (h2 : s.inc <+: s'.inc) (h3 : s.data <+: s'.data)
    (h4 : s.q1 <+: s'.q1) {v : List (WNext α n)}
    (h : CombOut (WriteNext.nextDep α s) (WriteNext.nextFun α) (WriteNext.nextLen α s) dmin dmax v) :
    CombOut (WriteNext.nextDep α s') (WriteNext.nextFun α) (WriteNext.nextLen α s') dmin dmax v :=
...
theorem _root_.Graphiti.AsyncFifo.Contracts.RegOutG.mono_bus {kq su : Nat} {G : Nat → Prop} {rclk rclk' : List Bool}
...
theorem _root_.Graphiti.AsyncFifo.Contracts.BusRegOutG.mono_bus {kq su : Nat} {G : Nat → Prop} {rclk rclk' : List Bool}
...
theorem _root_.Graphiti.AsyncFifo.Contracts.MemOutG.mono_bus {kq su : Nat} {G : Nat → Prop} {rclk rclk' : List Bool}
...
theorem _root_.Graphiti.AsyncFifo.Contracts.RegOutG.mono_guard {kq su : Nat} {G G' : Nat → Prop} {rclk : List Bool}
...
theorem _root_.Graphiti.AsyncFifo.Contracts.BusRegOutG.mono_guard {kq su : Nat} {G G' : Nat → Prop} {rclk : List Bool}
...
theorem _root_.Graphiti.AsyncFifo.Contracts.MemOutG.mono_guard {kq su : Nat} {G G' : Nat → Prop} {rclk : List Bool}
...
theorem spec_in_clk (v : List Bool) (h : s.clk ⊏ v) :
    ((wdomSpec α n lat stl su kq P S R pw).inputs.getIO ↑"clk").2 s v { s with clk := v } := by
  rw [PortMap.rw_rule_execution (by dsimp [reducePortMapgetIO])]; exact ⟨h, rfl⟩
theorem spec_in_inc (v : List Bool) (h : s.inc ⊏ v) :
    ((wdomSpec α n lat stl su kq P S R pw).inputs.getIO ↑"inc").2 s v { s with inc := v } := by
  rw [PortMap.rw_rule_execution (by dsimp [reducePortMapgetIO])]; exact ⟨h, rfl⟩
theorem spec_in_data (v : List α) (h : s.data ⊏ v) :
    ((wdomSpec α n lat stl su kq P S R pw).inputs.getIO ↑"data").2 s v { s with data := v } := by
  rw [PortMap.rw_rule_execution (by dsimp [reducePortMapgetIO])]; exact ⟨h, rfl⟩
theorem spec_in_rgray (v : List (BitVec (n+1))) (h : s.rgray ⊏ v) :
    ((wdomSpec α n lat stl su kq P S R pw).inputs.getIO ↑"rgray").2 s v { s with rgray := v } := by
  rw [PortMap.rw_rule_execution (by dsimp [reducePortMapgetIO])]; exact ⟨h, rfl⟩
theorem spec_in_orc (v : List (Orc n)) (h : s.orc ⊏ v) :
    ((wdomSpec α n lat stl su kq P S R pw).inputs.getIO ↑"orc").2 s v { s with orc := v } := by
  rw [PortMap.rw_rule_execution (by dsimp [reducePortMapgetIO])]; exact ⟨h, rfl⟩

theorem spec_out_gray (v : List (BitVec (n+1))) (h1 : s.gray_q <+: v)
    (h2 : WGrayF lat stl su kq P S R pw s.clk s.inc s.data s.rgray s.orc v) :
    ((wdomSpec α n lat stl su kq P S R pw).outputs.getIO ↑"gray").2 s v { s with gray_q := v } := by
...
theorem spec_out_full (v : List Bool) (h1 : s.full_q <+: v)
    ...
    ((wdomSpec α n lat stl su kq P S R pw).outputs.getIO ↑"full").2 s v { s with full_q := v } := by
  rw [PortMap.rw_rule_execution (by dsimp [reducePortMapgetIO])]; exact ⟨h1, h2, rfl⟩
theorem spec_out_mem (v : List (BitVec n → α)) (h1 : s.mem_q <+: v)
    (h2 : WMemF lat stl su kq P S R pw s.clk s.inc s.data s.rgray s.orc v) :
    ((wdomSpec α n lat stl su kq P S R pw).outputs.getIO ↑"mem").2 s v { s with mem_q := v } := by
  rw [PortMap.rw_rule_execution (by dsimp [reducePortMapgetIO])]; exact ⟨h1, h2, rfl⟩
...
theorem int_sync_next {out : List (BitVec (n+1))}
    (hout : out <+: SyncStage.syncOut lat su stl sclk sd sorc) (h : nq1 ⊏ out) :
...
theorem int_next_regs (v : List (WNext α n)) (h1 : nd <+: v)
    (h2 : CombOut (WriteNext.nextDep α ⟨nst, ninc, ndata, nq1, nd⟩) (WriteNext.nextFun α) (WriteNext.nextLen α ⟨nst, ninc, ndata, nq1, nd⟩) dmin dmax v)
    (h3 : rd ⊏ v) :
...
theorem refines_ψ (hP1 : kq + su + dmax + 2 ≤ P) (hP2 : stl + su + dmax + 2 ≤ P) (hS : su + dmax + 1 ≤ S)
    ...
    wdomTimed α n lat kq su stl dmin dmax Pg pwg Rg Rc ⊑_{ψ lat kq su stl dmin dmax P S R pw Pg pwg Rg Rc} wdomSpec α n lat stl su kq P S R pw := by
  intro i s Hψ
  ...
  · -- Input rules
    ...
    all_goals simp only [Prod.mk.injEq, WriteNext.NextSt.mk.injEq, WriteBank.RegSt.mk.injEq, and_assoc] at Hrule
...
theorem refines_initial :
    Module.refines_initial (wdomTimed α n lat kq su stl dmin dmax Pg pwg Rg Rc) (wdomSpec α n lat stl su kq P S R pw)
      (ψ lat kq su stl dmin dmax P S R pw Pg pwg Rg Rc) := by
  intro i hi
  obtain ⟨csrc, ⟨sclk, sd, sorc, sq⟩, F, ⟨nst, ninc, ndata, nq1, nd⟩, ⟨rclk, rd, crn, stq, fq, gq, mq⟩⟩ := i
  simp only [wdomTimed, Prod.mk.injEq, WriteNext.NextSt.mk.injEq, WriteBank.RegSt.mk.injEq] at hi
...
theorem wdomTimed_refines (hP1 : kq + su + dmax + 2 ≤ P) (hP2 : stl + su + dmax + 2 ≤ P) (hS : su + dmax + 1 ≤ S)
    (hdd : dmin ≤ dmax) (hR : dmax + su + 1 ≤ R) (hPg : Pg ≤ P) (hpwg : pwg ≤ pw) (hRg : Rg ≤ R) :
    wdomTimed α n lat kq su stl dmin dmax Pg pwg Rg Rc ⊑ wdomSpec α n lat stl su kq P S R pw :=
