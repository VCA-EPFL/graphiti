...
says what that wire *is*: the register contract of `ProofWriteOnly/Timed.lean` -- `RegOut` as written there,
...
`ProofWriteOnly/Timed.lean` states it.  The alternative was to thread a startup allowance up through
...
and that the conclusions are the contracts of `ProofWriteOnly/Timed.lean` rather than her filtered stream.
-/
...
`ProofWriteOnly/Timed.lean` writes it, at clock-to-q `4` and setup `1`. -/
