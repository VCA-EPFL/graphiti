...
* `clear_src` --- the reset source.  `clearSrc`'s contract is `ClearOK Rc`: the clear is
...
def c_emptyOf : TypedTemplate := cell "emptyOf" [("q",7)] [("empty",1)]
  "assign empty = ~q[3];   // the read state register stores !empty (ReadState.stBit)"
...
/-- The reset source.  `clearSrc`'s contract is `ClearOK Rc`: the clear is held low, is
...
def m_streg : Option String := build_unit "streg" stEnv WriteState.stLowered st_if
def m_stregr : Option String := build_unit "streg_r" stEnv ReadState.stLowered st_if
...
def m_mem : Option String := build_unit "mem" memEnv RegFile.memLowered mem_if
...
def m_bank : Option String := build_unit "bank" bankEnv WriteBank.bankLowered bank_if
...
def m_bankr : Option String := build_unit "bankr" bankREnv ReadBank.bankLowered bankr_if
...
def m_next : Option String := build_unit "next_state" nextEnv WriteNext.gateNextExpr next_if2
...
def m_nextr : Option String := build_unit "next_state_r" nextREnv ReadNext.gateNextRExpr nextr_if
...
def m_mux : Option String := build_unit "read_mux" muxEnv ReadPort.muxLowered mux_if
...
def wdomEnv : IdentMap String TypedTemplate :=
  [("clkF", c_fork2), ("WriteBank", s_bank), ("clearSrc", c_clearSrc), ("WriteNext", s_next), ("SyncStage", s_stage)].toAssocList
def wdom_if2 : VerilogInterface :=
  ifc [("clk",1),("inc",1),("data",1),("rgray",3),("orc",6)] [("gray",3),("full",1),("mem",4)]
def m_wdom : Option String := build_unit "wdom" wdomEnv wdomLowered wdom_if2

def rdomEnv : IdentMap String TypedTemplate :=
  [("clkF", c_fork2), ("ReadBank", s_bankr), ("clearSrc", c_clearSrc), ("ReadNext", s_nextr), ("SyncStage", s_stage), ("stF", c_fork2_7), ("ReadPort", s_mux)].toAssocList
def rdom_if2 : VerilogInterface :=
  ifc [("clk",1),("inc",1),("wgray",3),("orc",6),("mem",4)] [("gray",3),("empty",1),("rdata",1)]
def m_rdom : Option String := build_unit "rdom" rdomEnv rdomLowered rdom_if2
