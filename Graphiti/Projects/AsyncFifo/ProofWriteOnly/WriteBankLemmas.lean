...
/-! # `WriteBank`: the lemmas

Facts about the definitions in `components/level6/WriteBank.lean`.  The statement of the
...
@[drenv] theorem kenv_streg : kenv.find? "streg" = .some ⟨_, WriteState.stSpec⟩ := rfl
@[drenv] theorem kenv_forkSt : kenv.find? "forkSt" = .some ⟨_, fork2 (WSt 2)⟩ := rfl
...
@[drenv] theorem kenv_memory : kenv.find? "memory" = .some ⟨_, RegFile.memSpec⟩ := rfl
...
theorem stLen_eq (clk : List Bool) (d : List (WNext Bool 2)) (crn : List Bool) :
    WriteState.stLen clk (stOf d) crn = bankLen clk d crn := by simp [WriteState.stLen, bankLen]
...
theorem memLen_eq (clk : List Bool) (d : List (WNext Bool 2)) (crn : List Bool) :
    RegFile.memLen clk (weOf d) (addrOf d) (dataOf d) crn = bankLen clk d crn := by
  simp [RegFile.memLen, bankLen]
...
theorem bank_st (H : BankOK clk d crn R) {v : List (WSt 2)}
    (hv : v <+: WriteState.stOut clk (stOf d) crn) :
    RegOut 4 1 default clk (fun u => (d.getD u default).st) d.length v := by
  have h := WriteState.stOut_regOut (clk := clk) (d := stOf d) (crn := crn) (R := R) (by have := H.hR; omega)
...
theorem bank_mem (H : BankOK clk d crn R) {v : List (BitVec 2 → Bool)}
    (hv : v <+: RegFile.memOut clk (weOf d) (addrOf d) (dataOf d) crn) :
    MemOut 4 8 clk (fun u => (d.getD u default).we) (fun u => (d.getD u default).addr)
      (fun u => (d.getD u default).data) d.length v := by
  have h := RegFile.memOut_memOut (clk := clk) (we := weOf d) (addr := addrOf d) (data := dataOf d)
    (crn := crn) (R := R)
...
point is why `RegFile.memOut` reports against the file's own inputs rather than against its cells:
...
theorem bank_stG (hR : 6 ≤ R) {v : List (WSt 2)} (hv : v <+: WriteState.stOut clk (stOf d) crn) :
  ...
  rw [WriteState.stOut_length, stLen_eq] at hl
  ...
  have hcut := bank_st (bankOK_take hR htb G)
    (v := WriteState.stOut (clk.take t) (stOf (d.take t)) (crn.take t)) List.prefix_rfl
  have hcl : (WriteState.stOut (clk.take t) (stOf (d.take t)) (crn.take t)).length = t + 1 := by
    rw [WriteState.stOut_length, stLen_eq, hlt]
  have hp : WriteState.stOut (clk.take t) (stOf (d.take t)) (crn.take t) <+: WriteState.stOut clk (stOf d) crn :=
    WriteState.stOut_mono (List.take_prefix _ _) (stOf_mono (List.take_prefix _ _)) (List.take_prefix _ _)
...
theorem bank_memG (hR : 6 ≤ R) {v : List (BitVec 2 → Bool)}
  ...
  have hl := hv.length_le
  rw [RegFile.memOut_length, memLen_eq] at hl
  refine ⟨hl, fun t ht G => ?_⟩
  have htb : t ≤ bankLen clk d crn := by omega
  have hlt := bankLen_take (d := d) htb
  have hcut := bank_mem (bankOK_take hR htb G)
    (v := RegFile.memOut (clk.take t) (weOf (d.take t)) (addrOf (d.take t)) (dataOf (d.take t)) (crn.take t))
    List.prefix_rfl
  have hcl : (RegFile.memOut (clk.take t) (weOf (d.take t)) (addrOf (d.take t)) (dataOf (d.take t))
      (crn.take t)).length = t + 1 := by rw [RegFile.memOut_length, memLen_eq, hlt]
  have hp : RegFile.memOut (clk.take t) (weOf (d.take t)) (addrOf (d.take t)) (dataOf (d.take t)) (crn.take t)
      <+: RegFile.memOut clk (weOf d) (addrOf d) (dataOf d) crn :=
    RegFile.memOut_mono (List.take_prefix _ _) (weOf_mono (List.take_prefix _ _))
...
@[drcomponents]
def bankExact : StringModule (List Bool × List (WNext Bool 2) × List Bool × List (WSt 2) ×
  ...
  { inputs := [ (↑"clk", ⟨List Bool, fun s v s' => s.1 ⊏ v ∧ s' = (v, s.2)⟩)
    ...
    outputs := [ (↑"st", ⟨List (WSt 2), fun s v s' => s.2.2.2.1 <+: v ∧
                    v <+: WriteState.stOut s.1 (stOf s.2.1) s.2.2.1 ∧
                    s' = (s.1, s.2.1, s.2.2.1, v, s.2.2.2.2)⟩)
               , (↑"full", ⟨List Bool, fun s v s' => s.2.2.2.2.1 <+: v ∧
                    v <+: (WriteState.stOut s.1 (stOf s.2.1) s.2.2.1).map (·.full) ∧
               ...
               , (↑"mem", ⟨List (BitVec 2 → Bool), fun s v s' => s.2.2.2.2.2.2 <+: v ∧
                    v <+: RegFile.memOut s.1 (weOf s.2.1) (addrOf s.2.1) (dataOf s.2.1) s.2.2.1 ∧
