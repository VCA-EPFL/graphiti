...
This file relates the streams flowing between the blocks of the write domain's implementation (`wdomImpl`) to the
...
theorem syncRun_eq {sclk : List Bool} (hs : sclk <+: F) {x : Nat} (hx : x ≤ sclk.length) :
  ...
  | succ x ih =>
    ...
    show _ = (⟨(wstep stl (wRun lat stl su F inc data sd sorc x) (winp lat su F inc data sd sorc x)).q1,
              (wstep stl (wRun lat stl su F inc data sd sorc x) (winp lat su F inc data sd sorc x)).since⟩ : SyncStage.SyncReg n)
    ...
    · rw [wstep_rise h]
      ...
      simp [SyncStage.syncStep, SyncStage.syncInp, hr, h', winp]
...
structure Links : Prop where
  ...
  d_ok : CombOut (WriteNext.nextDep α ⟨nst, inc, data, nq1, nd⟩) (WriteNext.nextFun α) (WriteNext.nextLen α ⟨nst, inc, data, nq1, nd⟩) dmin dmax nd
...
theorem Links.lens (L : Links kq su dmin dmax lat stl P S R pw F sclk rclk sd sorc inc data nst nq1 nd rd st_q) :
  ...
  simp only [WriteNext.nextLen] at h2
...
theorem edge_value (L : Links kq su dmin dmax lat stl P S R pw F sclk rclk sd sorc inc data nst nq1 nd rd st_q)
  ...
  have hdep : ∀ y, e - su - 1 - dmax ≤ y → y ≤ e - dmin →
      WriteNext.nextDep α ⟨nst, inc, data, nq1, nd⟩ y =
    ...
    simp only [WriteNext.nextDep]
    rw [c1, c2, c3, c4]
  rw [L.d_link.getD_eq_left (by lia)]
  have hlen : u - dmin < WriteNext.nextLen α ⟨nst, inc, data, nq1, nd⟩ := by
    simp only [WriteNext.nextLen]; omega
...
theorem mem_correct (L : Links kq su dmin dmax lat stl P S R pw F sclk rclk sd sorc inc data nst nq1 nd rd st_q)
  ...
  have hW : ∀ e, e < t →
      (Contracts.WriteEdge a rclk (fun u => (rd.getD u default).we) (fun u => (rd.getD u default).addr)
