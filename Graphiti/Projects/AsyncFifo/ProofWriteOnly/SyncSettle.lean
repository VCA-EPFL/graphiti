...
`SyncStage.syncSpec` is where this development writes down the metastability assumption, and it
...
settling (`Orc.junk`), and `SyncStage.syncOut` is the resulting deterministic stream.  That is more than
...
--- the packed bus **is** `SyncStage.syncOut` for an oracle read off those three bits (`settle_orc`):
...
Nothing here weakens `SyncStage.syncSpec`: the theorem produces the oracle, it does not consume one.
-/
...
theorem syncStep_rise {s : SyncStage.SyncReg n} {i : SyncStage.SyncIn n} (h : i.rise = true) :
    SyncStage.syncStep s i = ⟨mix i.orc.sel i.dNew i.dOld, 0⟩ := by simp [SyncStage.syncStep, h]

theorem syncStep_norise {s : SyncStage.SyncReg n} {i : SyncStage.SyncIn n} (h : i.rise = false) :
    SyncStage.syncStep s i = ⟨s.val, s.since + 1⟩ := by simp [SyncStage.syncStep, h]

/-- The `since` counter of the stage is the counter of `SinceInv`. -/
theorem syncRun_since (t : Nat) :
    SinceInv stl clk t (SyncStage.syncRun lat su stl clk d orc t).since := by
  induction t with
  | zero => exact SinceInv.zero _ _
  | succ t ih =>
    show SinceInv stl clk (t + 1)
      (SyncStage.syncStep (SyncStage.syncRun lat su stl clk d orc t) (SyncStage.syncInp lat su clk d orc t)).since
    ...
    · have hr' : riseAt clk t = false := by simpa using hr
      rw [syncStep_norise (i := SyncStage.syncInp lat su clk d orc t) hr']
...
theorem syncRun_noedge {t : Nat} (h : NoEdge clk t) :
    SyncStage.syncRun lat su stl clk d orc t = ⟨0#(n+1), stl + t⟩ := by
  induction t with
  | zero => rfl
  | succ t ih =>
    have h' : NoEdge clk t := fun e he => h e (by lia)
    have hr : riseAt clk t = false := h t (Nat.lt_succ_self t)
    show SyncStage.syncStep (SyncStage.syncRun lat su stl clk d orc t) (SyncStage.syncInp lat su clk d orc t) = _
    rw [syncStep_norise (i := SyncStage.syncInp lat su clk d orc t) hr, ih h']
    rfl
...
theorem syncRun_at_edge {e : Nat} (hre : riseAt clk e = true) :
    SyncStage.syncRun lat su stl clk d orc (e + 1) =
      ⟨mix (orc.getD e default).sel (delayed lat d e) (delayed lat d (e - su - 1)), 0⟩ := by
  show SyncStage.syncStep (SyncStage.syncRun lat su stl clk d orc e) (SyncStage.syncInp lat su clk d orc e) = _
  rw [syncStep_rise (i := SyncStage.syncInp lat su clk d orc e) hre]
  rfl

/-- Between edges only the counter moves. -/
theorem syncRun_lastEdge {e t : Nat} (h : LastEdge clk e t) :
    SyncStage.syncRun lat su stl clk d orc t =
      ⟨(SyncStage.syncRun lat su stl clk d orc (e + 1)).val, t - e - 1⟩ := by
  ...
  | succ t ih =>
    ...
    · have hr : riseAt clk t = false := hno t hlt (Nat.lt_succ_self t)
      ...
      show SyncStage.SyncReg.mk _ _ = SyncStage.SyncReg.mk _ _
      congr 1
      lia
    · subst heq
      have h0 : (SyncStage.syncRun lat su stl clk d orc (e + 1)).since = 0 := by
        rw [syncRun_at_edge hre]
      have he0 : e + 1 - e - 1 = 0 := by lia
      rw [he0]
      generalize SyncStage.syncRun lat su stl clk d orc (e + 1) = r at *
      obtain ⟨val, since⟩ := r
      simp only at h0
      subst h0
      rfl
...
packed into a bus, are exactly what `SyncStage.syncSpec` promises --- for any oracle that reports,
...
theorem settle_orc_of {v : List (BitVec 3)} {orc : List (Orc 2)}
    ...
    v = SyncStage.syncOut lat su stl clk d orc := by
  have hsl : SyncStage.syncLen lat clk d orc = v.length := by
    unfold SyncStage.syncLen
    rw [horcl]
    exact Nat.min_eq_right hlen
  have houtlen : (SyncStage.syncOut lat su stl clk d orc).length = v.length := by
    unfold SyncStage.syncOut; rw [timeline_length, hsl]
  ...
  have hval : ∀ t, t < v.length → v.getD t 0#3 =
      (if (SyncStage.syncRun lat su stl clk d orc t).since < stl
       then (orc.getD t default).junk else (SyncStage.syncRun lat su stl clk d orc t).val) := by
    intro t ht
    rcases syncRun_since (lat := lat) (su := su) (stl := stl) (d := d) (orc := orc) t with
      ⟨hno, hs⟩ | ⟨e, he, hs⟩
    · have hnot : ¬ ((SyncStage.syncRun lat su stl clk d orc t).since < stl) := by rw [hs]; lia
      ...
      rfl
    · have het := he.1
      have hsince : (SyncStage.syncRun lat su stl clk d orc t).since = t - e - 1 := by lia
...
theorem settle_orc {v : List (BitVec 3)}
    ...
    (hpack : ∀ t, t < v.length →
      v.getD t 0#3 = bv3 ((Q 0).getD t false) ((Q 1).getD t false) ((Q 2).getD t false)) :
    v = SyncStage.syncOut lat su stl clk d (orcOf lat su stl d Q v.length) := by
...
`SyncStage.syncSpec` is exhausted by three settling bits. -/
theorem settle_exists_orc {v : List (BitVec 3)}
    ...
    (hpack : ∀ t, t < v.length →
      v.getD t 0#3 = bv3 ((Q 0).getD t false) ((Q 1).getD t false) ((Q 2).getD t false)) :
    ∃ orc : List (Orc 2), orc.length = v.length ∧ v = SyncStage.syncOut lat su stl clk d orc :=
...
stage in the sense of `SyncStage.syncSpec`.  The oracle is not assumed, it is computed.

`v` is a prefix of what the register reports because `SyncStage.syncSpec` reports one instant less than a
register does: `SyncStage.syncLen` has no `+ 1`. -/
theorem stage_settles {lat su stl kq : Nat} {clk crn : List Bool} {d : List (BitVec 3)}
    ...
    (hlen : v.length ≤ min clk.length (d.length + lat)) :
    ∃ orc : List (Orc 2), orc.length = v.length ∧ v = SyncStage.syncOut lat su stl clk d orc := by
...
theorem stage_settles_of_aperture {lat R : Nat} {clk crn : List Bool} {d : List (BitVec 3)}
    ...
    (hv : v <+: busOut clk (wireOf lat d) crn)
    (hlen : v.length ≤ min clk.length (d.length + lat)) :
    ∃ orc : List (Orc 2), orc.length = v.length ∧ v = SyncStage.syncOut lat 1 4 clk d orc := by
