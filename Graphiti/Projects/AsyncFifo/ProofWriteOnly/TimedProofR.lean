...
theorem rsyncRun_eq {sclk : List Bool} (hs : sclk <+: F) {x : Nat} (hx : x ≤ sclk.length) :
    SyncStage.syncRun lat su stl sclk sd sorc x =
      ⟨(rRun lat stl su F inc sd sorc x).q1, (rRun lat stl su F inc sd sorc x).since⟩ := by
  induction x with
  | zero => rfl
  | succ x ih =>
    have ih := ih (by lia)
    show SyncStage.syncStep (SyncStage.syncRun lat su stl sclk sd sorc x) (SyncStage.syncInp lat su sclk sd sorc x) = _
    rw [ih]
    show _ = (⟨(rstep stl (rRun lat stl su F inc sd sorc x) (rinp lat su F inc sd sorc x)).q1,
              (rstep stl (rRun lat stl su F inc sd sorc x) (rinp lat su F inc sd sorc x)).since⟩ : SyncStage.SyncReg n)
    ...
    · rw [rstep_rise h]
      ...
      simp [SyncStage.syncStep, SyncStage.syncInp, hr, h', rinp]
    · have h' : (rinp lat su F inc sd sorc x).rise = false := by simpa using h
      rw [rstep_norise h']
      have h'' : riseAt F x = false := h'
      simp [SyncStage.syncStep, SyncStage.syncInp, hr, h'']
...
structure RLinks : Prop where
  ...
  d_ok : CombOut (ReadNext.rnextDep ⟨nst, inc, nq1, nd⟩) ReadNext.rnextFun (ReadNext.rnextLen ⟨nst, inc, nq1, nd⟩) dmin dmax nd
...
theorem RLinks.lens (L : RLinks kq su dmin dmax lat stl P S R pw F sclk rclk sd sorc inc nst mst nq1 nd rd rmem mem st_q) :
  ...
  simp only [SyncStage.syncOut, timeline_length, SyncStage.syncLen] at h3
  simp only [ReadNext.rnextLen] at h2
...
theorem redge_value (L : RLinks kq su dmin dmax lat stl P S R pw F sclk rclk sd sorc inc nst mst nq1 nd rd rmem mem st_q)
    ...
    ∀ u, e - su - 1 ≤ u → u ≤ e → rd.getD u default =
      ReadNext.rnextFun ((rst (rRun lat stl su F inc sd sorc e)), inc.getD e false,
  ...
  have hdep : ∀ y, e - su - 1 - dmax ≤ y → y ≤ e - dmin →
      ReadNext.rnextDep ⟨nst, inc, nq1, nd⟩ y =
    ...
    have c4 : nq1.getD y 0 = (rRun lat stl su F inc sd sorc e).q1 := by
      ...
      rw [timeline_getD _ (by unfold SyncStage.syncLen; omega)]
      rw [rsyncRun_eq lat stl su F inc sd sorc L.sclk_F (by lia)]
      simp only
      rw [if_neg (by lia), hq1]
    simp only [ReadNext.rnextDep]
    rw [c1, c2, c4]
  rw [L.d_link.getD_eq_left (by lia)]
  have hlen : u - dmin < ReadNext.rnextLen ⟨nst, inc, nq1, nd⟩ := by
    simp only [ReadNext.rnextLen]; omega
...
theorem rreg_value (L : RLinks kq su dmin dmax lat stl P S R pw F sclk rclk sd sorc inc nst mst nq1 nd rd rmem mem st_q)
  ...
  · have he1 := he.1
    ...
    show (ReadNext.rnextFun _).st = rst (rstep stl (rRun lat stl su F inc sd sorc e) (rinp lat su F inc sd sorc e))
    rw [rstep_rise (i := rinp lat su F inc sd sorc e) hreF]
    simp [ReadNext.rnextFun, rNext, rst, rinp, hsn]
...
theorem rgray_correct (L : RLinks kq su dmin dmax lat stl P S R pw F sclk rclk sd sorc inc nst mst nq1 nd rd rmem mem st_q)
  ...
  have hval : ∀ t, t < gray_q.length → RFilter P S R pw F inc t → Settled kq rclk t →
    ...
    · have he1 := he.1
      ...
      rw [hev e (by lia) (Nat.le_refl e), rRun_lastEdge _ _ _ _ _ _ _ heF]
      show (ReadNext.rnextFun _).gnext = gray (rstep stl (rRun lat stl su F inc sd sorc e) (rinp lat su F inc sd sorc e)).ptr
      rw [rstep_rise (i := rinp lat su F inc sd sorc e) hreF]
      simp [ReadNext.rnextFun, rNext, rst, rinp]
  ...
  · left
    ...
    show ((ReadNext.rnextFun _).gnext).getLsbD i =
