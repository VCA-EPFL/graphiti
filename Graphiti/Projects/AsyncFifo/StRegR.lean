...
record split into bits on the way in and reassembled on the way out --- `StReg.lean` at the read
domain's record.  The two files are line-for-line the same apart from the one bit below; see
`StReg.lean` for why they are not one register generic in the record.
...
and `stBit_all` puts the flag back the right way up.  Nothing outside this file sees the
inversion --- `stOut` reports `RSt 2` records, with `empty` meaning what it says.
-/

set_option linter.unusedSectionVars false
set_option linter.unusedSimpArgs false

namespace Graphiti.AsyncFifo.StRegR
...
theorem stBit_all (x : RSt 2) :
    (⟨bv3 (stBit 0 x) (stBit 1 x) (stBit 2 x), !(stBit 3 x),
      bv3 (stBit 4 x) (stBit 5 x) (stBit 6 x)⟩ : RSt 2) = x := by
  obtain ⟨ptr, empty, q2⟩ := x
  simp only [stBit, RSt.mk.injEq]
  refine ⟨?_, by simp, ?_⟩
...
@[simp] theorem stBit_default (i : Nat) : stBit i (default : RSt 2) = false := by
  unfold stBit
  split
...
@[simp] theorem bitsOf_length (i : Nat) (d : List (RSt 2)) : (bitsOf i d).length = d.length := by
  simp [bitsOf]

theorem bitsOf_mono {i : Nat} {d d' : List (RSt 2)} (h : d <+: d') : bitsOf i d <+: bitsOf i d' :=
  h.map _

theorem bitsOf_getD_all (i : Nat) (d : List (RSt 2)) (u : Nat) :
...
theorem stOut_mono {clk clk' : List Bool} {d d' : List (RSt 2)}
...
theorem spec_in_d (v : List (RSt 2)) (h : sp.2.1 ⊏ v) :
...
theorem spec_out_q (v : List (RSt 2)) (h1 : sp.2.2.2 <+: v)
