...
`Timed.syncReg` states the metastability assumption at the *bus* level: an oracle says, at every
...
`settleOut1_settleOut` checks that each of those primitives satisfies `Timed.SettleOut`, the
contract `Dff.dffOut_settleOut` proves of the real flip-flop wherever its data meets the
aperture.
-/
...
/-- The bus-level register of `Timed.syncReg`, bit by bit. -/
theorem syncRun_bit (i : Nat) (t : Nat) :
    (syncRun lat su stl clk d orc t).since
        = (sbitRun su stl clk (bitsOf i (wireOf lat d)) (selBit i orc) t).since ∧
      (syncRun lat su stl clk d orc t).val.getLsbD i
        = (sbitRun su stl clk (bitsOf i (wireOf lat d)) (selBit i orc) t).val := by
  induction t with
  | zero => exact ⟨rfl, by show (0#3).getLsbD i = false; simp⟩
  | succ t ih =>
    show (syncStep (syncRun lat su stl clk d orc t) (syncInp lat su clk d orc t)).since = _ ∧
      (syncStep (syncRun lat su stl clk d orc t) (syncInp lat su clk d orc t)).val.getLsbD i = _
    show _ ∧ _
    ...
    · have hr0 : riseAt clk t = false := by simpa using hr
      rw [syncStep_norise (i := syncInp lat su clk d orc t) hr0]
...
theorem sbitLen_eq (i : Nat) :
    sbitLen clk (bitsOf i (wireOf lat d)) (selBit i orc) (junkBit i orc)
      = syncLen lat clk d orc := by
  simp only [sbitLen, syncLen, bitsOf_length, wireOf_length, selBit_length, junkBit_length,
    Nat.min_self]

/-- **The bus-level stage is three one-bit stages.**  Nothing of the oracle is left over: `sel`
and `junk` split per bit, and `syncOut` is the three bits packed. -/
theorem syncOut_pack :
    syncOut lat su stl clk d orc =
  ...
  have hl : ∀ i, (settleOut1 su stl clk (bitsOf i (wireOf lat d)) (selBit i orc)
      (junkBit i orc)).length = syncLen lat clk d orc := by
  ...
  have hsl : (syncOut lat su stl clk d orc).length = syncLen lat clk d orc := by
    unfold syncOut; rw [timeline_length]
  refine (((prefix_iff_length_getD (0#3)).mpr ⟨by rw [hsl, hpl], fun t ht => ?_⟩) :
    syncOut lat su stl clk d orc <+: _).eq_of_length (by rw [hsl, hpl])
  rw [hsl] at ht
  unfold syncOut
...
itself, which `syncOut` counts as settling.
