...
/-!
# What the read domain's state register does in time

`StRegR.lean` shows that the seven flip-flops report the bits of `stOut`.  This file turns the
per-bit theorems of `DffTiming.lean` into `RegOut` for the record, exactly as
`StRegTiming.lean` does for the write domain: a record is its bits --- with the flag's bit read
back complemented, which is what `stBit_all` does.
-/
...
def stLen (clk : List Bool) (d : List (RSt 2)) (crn : List Bool) : Nat :=
  min (min clk.length d.length) crn.length

theorem dffLen_bits (i : Nat) (clk : List Bool) (d : List (RSt 2)) (crn : List Bool) :
    dffLen clk (bitsOf i d) crn = stLen clk d crn := by
  simp [dffLen, stLen]

@[simp] theorem stOut_length (clk : List Bool) (d : List (RSt 2)) (crn : List Bool) :
    (stOut clk d crn).length = stLen clk d crn + 1 := by
  simp only [stOut, packStOut_length, dffOut_length, dffLen_bits]
  omega

theorem stOut_getD {clk : List Bool} {d : List (RSt 2)} {crn : List Bool} {t : Nat}
...
variable {clk : List Bool} {d : List (RSt 2)} {crn : List Bool} {R : Nat}
...
theorem stOut_regOut (hR : 2 ≤ R) (hclear : ClearOK R crn (stLen clk d crn))
    (hreset : ResetOK (R + 3) clk (stLen clk d crn)) (hpulse : PulseOK 3 clk (stLen clk d crn))
    {v : List (RSt 2)} (hv : v <+: stOut clk d crn) :
