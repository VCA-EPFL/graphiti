...
`GateNext.gateNextExpr`, the very expression the refinement theorem is about.  What is still
...
netlist of gates (`GateNext.lean`, depth `4` and 1-bit data), and `gateNext_refines` is proved
...
def gateNextVerilog : Option String :=
  build_verilog_of_exprLow "next_state" gateEnv GateNext.gateNextExpr next_if
