...
the circuit (`Modules.lean`), so the refinement theorem quantifies over every resolution.
The wires themselves are exact.

The temporal theorem `fifo_correct` (`Invariant.lean`) shows that, as long as the clocks
