...
it is the same seven gates as every other flip-flop here, and `Sync.lean` builds it and proves
...
> "the first stage settles", so `syncNetlist ⊑ Timed.syncReg` is not merely unproved, it is
> false, and `dff_metastable` is the counterexample.

`Timed.syncReg` promises that `stl` instants after an edge the output is a per-bit mixture of
...
So the assumption has to be written down rather than discharged, and `Timed.syncReg` is where
...
*second* stage needs no such treatment: its data is `syncOut`, which is stable from `stl`
...
This is what makes `stl < P` -- the settling budget of `Timed.syncReg`, and the hypothesis that
