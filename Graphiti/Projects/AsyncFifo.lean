...
`Bank.lean` delivers them in that form (`bank_stG`, `bank_grayG`, `bank_memG`, `bank_fullG`)
...
The results, all with standard axioms and no `sorry`:
    ...
    Bank.bank_st/gray/mem/full : the four output relations of `Timed.regBank`
    Bank.bank_stG/grayG/memG/fullG : the same, per instant --- exactly what `regBank` asks
...
* `GateNext.lean` — generated by `gen/gen_gatenext.py`: the write domain's next-state logic for
...
* `GateNextR.lean` — generated by `gen/gen_gatenextr.py`: the same for the read domain, which is
...
* `GateRegs.lean` — generated by `gen/gen_gateregs.py`: the composition of the storage
...
* `GateSync.lean` — generated by `gen/gen_syncstage.py`: `stage_refines`, the synchroniser
...
* `Dff.lean` — generated by `gen/gen_dff.py`: the edge-triggered D flip-flop with asynchronous
...
* `BusReg.lean` — generated by `gen/gen_busreg.py`: three flip-flops as the three-bit Gray
...
* `EnReg.lean` — generated by `gen/gen_enreg.py`: one cell of the register file, a flip-flop
...
* `Bank.lean` — the write domain's register bank: the seven-bit state register, the three-bit
  ...
  (`bank_refines`).  `bank_st`, `bank_gray`, `bank_mem`, `bank_full` read the blocks' contracts
  back in the bank's terms — they are the four output relations of `Timed.regBank`.  The bank
  has one port the register-level model does not: the clear.
...
* `gen/` — the netlist generator, and `memprobe.py`, which profiles a Lean file declaration by
  declaration under a memory cap.
